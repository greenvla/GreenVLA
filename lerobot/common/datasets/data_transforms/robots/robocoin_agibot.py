...
class RobocoinAgibotInputsTransform(DataTransformFn):
    max_action_dim: int
    ...
    # Masks as lists
    left_arm_joints_mask = [0, 1, 2, 3, 4, 5, 6]
    right_arm_joints_mask = [7, 8, 9, 10, 11, 12, 13]
    left_arm_cartesian_mask = [14, 15, 16, 17, 18, 19, 20]
    right_arm_cartesian_mask = [21, 22, 23, 24, 25, 26, 27]
    left_gripper_mask = [32]
    right_gripper_mask = [33]
...
@dataclasses.dataclass(frozen=True)
class RobocoinAgibotOutputsTransform(DataTransformFn):
    action_dim: int
    def __call__(self, data: dict) -> dict:
