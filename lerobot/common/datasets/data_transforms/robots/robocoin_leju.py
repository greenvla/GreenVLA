...
class RobocoinLejuInputsTransform(DataTransformFn):
    max_action_dim: int
    ...
    right_arm_joints_mask = [7, 8, 9, 10, 11, 12, 13]  # 7 joints
    left_gripper_joints_mask = [26, 27] # originally there were 6 joints, but 5 of them are same
    # left_gripper_joints_mask = [26, 27, 28, 29, 30, 31]  # 6 joints (left_hand)
    right_gripper_joints_mask = [32, 33] # originally there were 6 joints, but 5 of them are same
    # right_gripper_joints_mask = [32, 33, 34, 35, 36, 37]  # 6 joints (right_hand)
    
    left_arm_cartesian_mask = [96, 97, 98, 102, 103, 104]  # pos xyz + rot euler xyz (6 values)
    right_arm_cartesian_mask = [99, 100, 101, 105, 106, 107]  # pos xyz + rot euler xyz (6 values)
...
@dataclasses.dataclass(frozen=True)
class RobocoinLejuOutputsTransform(DataTransformFn):
    action_dim: int
