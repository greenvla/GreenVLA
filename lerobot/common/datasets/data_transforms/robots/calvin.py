...
class CalvinInputsTransform(DataTransformFn):
    action_dim: int
    state_mask = [i for i in range(7)] #we observing only x y z roll pitch yaw gripper width
    action_mask = [i for i in range(6, 13)] # we use relative actions, dx dy dz droll dpitch dyaw gripper_action
    ...
    def __call__(self, data: dict) -> dict:
        ...
        state = pad_to_dim(state, self.action_dim, value=0.0).to(torch.float32)
        base_image = parse_image_helper(data["observation/rgb_static"])
        wrist_image = parse_image_helper(data["observation/rgb_gripper"])
        
        if isinstance(base_image, np.ndarray):
            base_image = torch.from_numpy(base_image).permute(2, 0, 1)
            wrist_image = torch.from_numpy(wrist_image).permute(2, 0, 1)
        ...
        if "actions" in data:
            ...
            inputs["actions"] = pad_to_dim(actions, self.action_dim, axis=-1, value=0.0).to(torch.float32)
...
@dataclasses.dataclass(frozen=True)
class CalvinOutputsTransform(DataTransformFn):
    def __call__(self, data: dict) -> dict:
