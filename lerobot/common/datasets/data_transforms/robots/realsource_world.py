...
class RealsourceWorldInputsTransform(DataTransformFn):
    max_action_dim: int
    ...
    left_gripper_mask = [7]  # 1 gripper
    right_arm_joints_mask = [8, 9, 10, 11, 12, 13, 14]  # 7 joints
    right_gripper_mask = [15]  # 1 gripper
    lift_mask = [16]  # 1 lift position
    left_arm_cartesian_mask = [43, 44, 45, 46, 47, 48, 49]  # pos xyz + quat wxyz (7 values)
    right_arm_cartesian_mask = [50, 51, 52, 53, 54, 55, 56]  # pos xyz + quat wxyz (7 values)
...
@dataclasses.dataclass(frozen=True)
class RealsourceWorldOutputsTransform(DataTransformFn):
    action_dim: int
