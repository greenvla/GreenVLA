...
class RobocoinRMCAidaInputsTransform(DataTransformFn):
    max_action_dim: int
    ...
    # Masks as lists
    right_arm_joints_mask = [0, 1, 2, 3, 4, 5, 6]  # 7 joints
    right_gripper_mask = [7]  # 1 gripper
    right_arm_cartesian_mask = [8, 9, 10, 11, 12, 13]  # pos xyz + rot euler xyz (6 values)
    left_arm_joints_mask = [14, 15, 16, 17, 18, 19, 20]  # 7 joints
    left_gripper_mask = [21]  # 1 gripper
    left_arm_cartesian_mask = [22, 23, 24, 25, 26, 27]  # pos xyz + rot euler xyz (6 values)
...
@dataclasses.dataclass(frozen=True)
class RobocoinRMCAidaOutputsTransform(DataTransformFn):
    action_dim: int
