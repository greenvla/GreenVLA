...
class BridgeInputsTransform(DataTransformFn):
    action_dim: int
    state_mask = [i for i in range(8) if i != 6]
...
@dataclasses.dataclass(frozen=True)
class BridgeOutputsTransform(DataTransformFn):
    def __call__(self, data: dict) -> dict:
        actions = data["actions"]
        if isinstance(actions, np.ndarray):
            if actions.ndim==2:
            ...
            if actions.ndim==2:
                return {"actions": actions[:, :7].cpu().numpy()}
            elif actions.ndim==3:
                return {"actions": actions[:, :, :7].cpu().numpy()}
            else:
                raise ValueError(f"Unsupported action shape: {actions.shape}")
